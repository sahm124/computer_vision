...
def instance_normalization(features):
    with tf.compat.v1.variable_scope(None, default_name='IN'):
...
def multi_layer_fc_fn(sv, mask=None, Ks=(64, 32, 64), num_classes=4,
    ...
    Args:
        ...
        receiving vertices. When a [T, DEGREE, M] tensor is provided, the
        degree of each receiving vertex is assumed to be same.
        N is the total number of edges, M is the length of features. T is
        the number of receiving vertices, DEGREE is the in-degree of each
        receiving vertices. When a [T, DEGREE, M] tensor is provided, the
        degree of each receiving vertex is assumed to be same.
...
class ClassAwarePredictor(object):
    ...
    def apply_regular(self, features, num_classes, box_encoding_len,
        ...
        with tf.compat.v1.variable_scope('predictor'):
            ...
            with tf.compat.v1.variable_scope('loc'):
                for class_idx in range(num_classes):
                    with tf.compat.v1.variable_scope('cls_%d' % class_idx):
...
class PointSetPooling(object):
    """A class to implement local graph neural network."""
    ...
    def apply_regular(self,
        ...
        Args:
            ...
            output_MLP_depth_list: a list of MLP units to embed set features.
        ...
        with tf.compat.v1.variable_scope('extract_vertex_features'):
        ...
        with tf.compat.v1.variable_scope('combined_features'):
...
class GraphNetAutoCenter(object):
    """A class to implement point graph neural network layer."""
