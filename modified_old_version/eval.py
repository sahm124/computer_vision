...
t_encoded_gt_boxes = tf.compat.v1.placeholder(dtype=tf.float32,
    shape=[None, 1, BOX_ENCODING_LEN])
t_valid_gt_boxes = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1, 1])
...
# optimizers ================================================================
global_step = tf.compat.v1.Variable(0, dtype=tf.int32, trainable=False)
fetches = {
    'step': global_step,
...
def eval_repeat():
    ...
    while True:
        ...
        model_path = tf.compat.v1.train.latest_checkpoint(eval_config['train_dir'])
        if not model_path:
