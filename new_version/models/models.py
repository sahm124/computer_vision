...
regularizer_dict = {
    'l2': lambda **kwargs: tf.keras.regularizers.l2(kwargs.get('l2', 0.01)),
    'l1': lambda **kwargs: tf.keras.regularizers.l1(kwargs.get('l1', 0.01)),
    'l1_l2': lambda **kwargs: tf.keras.regularizers.l1_l2(
        l1=kwargs.get('l1', 0.01), l2=kwargs.get('l2', 0.01)),
}
...
class MultiLayerFastLocalGraphModelV2(object):
    ...
    def predict(self,
        ...
        with tf.name_scope(predictor_config['scope']):
    ...
    def loss(self, logits, labels, pred_box, gt_box, valid_box,
        ...
        loss_dict = {}
        assert cls_loss_type in ['softmax', 'top_k_softmax', 'focal_sigmoid', 'focal_softmax']
        ...
        if loc_loss_type == 'huber_loss':
            all_loc_loss = loc_loss_weight * tf.compat.v1.losses.huber_loss(
                gt_box,
                pred_box,
                delta=1.0,
                weights=valid_box,
            ...
            if 'classwise_loc_loss_weight' in loc_loss_kwargs and self._mode == 'train':
                classwise_loc_loss_weight = loc_loss_kwargs['classwise_loc_loss_weight']
                classwise_loc_loss_weight = tf.gather(classwise_loc_loss_weight, labels)
            ...
            loc_loss = tf.math.divide_no_nan(tf.reduce_sum(mean_loc_loss), num_valid_endpoint)
            classwise_loc_loss = []
            for class_idx in range(self.num_classes):
                class_mask = tf.where(tf.equal(tf.squeeze(labels, axis=1), tf.constant(class_idx, tf.int32)))
        ...
        if loc_loss_type == 'top_k_huber_loss':
            ...
            top_k_loc_loss, top_k_indices = tf.math.top_k(loc_loss, k=k, sorted=True)
            ...
            loc_loss = tf.math.divide_no_nan(tf.reduce_sum(top_k_loc_loss), num_valid_endpoint)
            top_k_labels = tf.gather(labels, top_k_indices)
            all_top_k_loc_loss = tf.gather(all_loc_loss, top_k_indices)
            classwise_loc_loss = []
            for class_idx in range(self.num_classes):
                class_mask = tf.where(tf.equal(tf.squeeze(top_k_labels), tf.constant(class_idx, tf.int32)))
                l = tf.reduce_sum(tf.gather(all_top_k_loc_loss, class_mask), axis=0)
                l = tf.squeeze(l, axis=0)
                is_nan_mask = tf.math.is_nan(l)
                l = tf.where(is_nan_mask, tf.zeros_like(l), l)
                classwise_loc_loss.append(l)
        ...
        reg_loss = tf.reduce_sum(tf.compat.v1.losses.get_regularization_losses())
        loss_dict.update({
            'cls_loss': cls_loss,
            'loc_loss': loc_loss,
            'reg_loss': reg_loss,
...
def get_model(model_name):
    """Fetch a model class."""
    model_map = {
        'multi_layer_fast_local_graph_model_v2': MultiLayerFastLocalGraphModelV2,
    }
