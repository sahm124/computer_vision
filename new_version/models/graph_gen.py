...
def multi_layer_downsampling(points_xyz, base_voxel_size, levels=[1], add_rnd3d=False):
    ...
    for level in levels:
        if np.isclose(last_level, level):
            ...
            if add_rnd3d:
                xyz_idx = (points_xyz - xyz_offset + base_voxel_size * level * np.random.random((1, 3))) // (base_voxel_size * level)
                ...
                downsampled_xyz = np.add.reduceat(sorted_points_xyz, indices, axis=0) / lens[:, np.newaxis]
                downsampled_list.append(np.array(downsampled_xyz))
            else:
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(points_xyz)
                downsampled_xyz = np.asarray(o3d.geometry.voxel_down_sample(pcd, voxel_size=base_voxel_size * level).points)
...
def multi_layer_downsampling_select(points_xyz, base_voxel_size, levels=[1], add_rnd3d=False):
    ...
    for i in range(1, num_levels):
        ...
        if np.isclose(current_level, last_level):
            ...
            keypoint_indices_list.append(np.expand_dims(np.arange(base_points.shape[0]), axis=1))
        else:
...
def multi_layer_downsampling_random(points_xyz, base_voxel_size, levels=[1], add_rnd3d=False):
    ...
    for level in levels:
        ...
        if np.isclose(last_level, level):
            ...
            if not add_rnd3d:
                ...
                xyz_idx = (last_points_xyz - xyz_offset + base_voxel_size * level * np.random.random((1, 3))) // (base_voxel_size * level)
            xyz_idx = xyz_idx.astype(np.int32)
            dim_x, dim_y, dim_z = np.amax(xyz_idx, axis=0) + 1
            keys = xyz_idx[:, 0] + xyz_idx[:, 1] * dim_x + xyz_idx[:, 2] * dim_y * dim_x
            ...
            keypoint_indices_list.append(np.expand_dims(np.array(downsampled_xyz_idx), axis=1))
        last_level = level

    return vertex_coord_list, keypoint_indices_list

def gen_multi_level_local_graph_v3(points_xyz, base_voxel_size, level_configs, add_rnd3d=False, downsample_method='center'):
...
def gen_disjointed_rnn_local_graph_v3(points_xyz, center_xyz, radius, num_neighbors, neighbors_downsample_method='random', scale=None):
    ...
    if num_neighbors > 0:
        if neighbors_downsample_method == 'random':
            indices = [neighbors if neighbors.size <= num_neighbors else np.random.choice(neighbors, num_neighbors, replace=False) for neighbors in indices]
...
def gen_knn_and_radius_neighbors_graph(points_xyz, center_xyz, radius, k, neighbors_downsample_method='random', scale=None):
    ...
    nbrs_radius = NearestNeighbors(radius=radius, algorithm='ball_tree', n_jobs=1).fit(points_xyz)
    radius_indices = nbrs_radius.radius_neighbors(center_xyz, return_distance=False)

    # Find k-nearest neighbors
    nbrs_knn = NearestNeighbors(n_neighbors=k, algorithm='ball_tree', n_jobs=1).fit(points_xyz)
    ...
    vertices_i = np.concatenate([i * np.ones(neighbors.size, dtype=np.int32) for i, neighbors in enumerate(final_indices)])
    vertices = np.array([vertices_v, vertices_i]).transpose()
    return vertices
