...
parser.add_argument('--dataset_root_dir', type=str, default='../dataset/kitti/', help='Path to KITTI dataset. Default="../dataset/kitti/"')
parser.add_argument('--dataset_split_file', type=str, default='', help='Path to KITTI dataset split file. Default="DATASET_ROOT_DIR/3DOP_splits/train_config["train_dataset"]"')
...
def fetch_data(frame_idx):
    ...
    if 'crop_aug' in train_config:
        cam_rgb_points, box_label_list = sampler.crop_aug(cam_rgb_points, box_label_list, sample_rate=train_config['crop_aug']['sample_rate'], parser_kwargs=train_config['crop_aug']['parser_kwargs'])
    ...
    if config['label_method'] == 'yaw':
        cls_labels, boxes_3d, valid_boxes, label_map = dataset.assign_classaware_label_to_points(box_label_list, last_layer_points_xyz, expend_factor=train_config.get('expend_factor', (1.0, 1.0, 1.0)))
    if config['label_method'] == 'Car':
        cls_labels, boxes_3d, valid_boxes, label_map = dataset.assign_classaware_car_label_to_points(box_label_list, last_layer_points_xyz, expend_factor=train_config.get('expend_factor', (1.0, 1.0, 1.0)))
    if config['label_method'] == 'Pedestrian_and_Cyclist':
        cls_labels, boxes_3d, valid_boxes, label_map = dataset.assign_classaware_ped_and_cyc_label_to_points(box_label_list, last_layer_points_xyz, expend_factor=train_config.get('expend_factor', (1.0, 1.0, 1.0)))
...
def batch_data(batch_list):
    ...
    for level_idx in range(level_num - 1):
        ...
        for batch_idx in range(batch_size):
            ...
            vertices.append(np.hstack([N_edges_list[batch_idx][level_idx][:, [0]] + point_counter, N_edges_list[batch_idx][level_idx][:, [1]] + center_counter]))
    ...
    return (batched_input_v, batched_vertex_coord_list, batched_keypoint_indices_list, batched_edges_list, batched_cls_labels, batched_encoded_boxes, batched_valid_boxes)
...
for gi in range(NUM_GPU):
    with tf.device('/gpu:%d' % gi):
        for cp_idx in range(COPY_PER_GPU):
            if config['input_features'] == 'irgb':
                t_initial_vertex_features = tf.keras.Input(shape=(None, 4), dtype=tf.float32)
            elif config['input_features'] == 'rgb':
                t_initial_vertex_features = tf.keras.Input(shape=(None, 3), dtype=tf.float32)
            elif config['input_features'] == '0000':
                t_initial_vertex_features = tf.keras.Input(shape=(None, 4), dtype=tf.float32)
            elif config['input_features'] == 'i000':
                t_initial_vertex_features = tf.keras.Input(shape=(None, 4), dtype=tf.float32)
            elif config['input_features'] == 'i':
                t_initial_vertex_features = tf.keras.Input(shape=(None, 1), dtype=tf.float32)
            elif config['input_features'] == '0':
                t_initial_vertex_features = tf.keras.Input(shape=(None, 1), dtype=tf.float32)

            t_vertex_coord_list = [tf.keras.Input(shape=(None, 3), dtype=tf.float32)]
            for _ in range(len(config['graph_gen_kwargs']['level_configs'])):
                t_vertex_coord_list.append(tf.keras.Input(shape=(None, 3), dtype=tf.float32))

            t_edges_list = []
            for _ in range(len(config['graph_gen_kwargs']['level_configs'])):
                t_edges_list.append(tf.keras.Input(shape=(None, None), dtype=tf.int32))

            t_keypoint_indices_list = []
            for _ in range(len(config['graph_gen_kwargs']['level_configs'])):
                t_keypoint_indices_list.append(tf.keras.Input(shape=(None, 1), dtype=tf.int32))

            t_class_labels = tf.keras.Input(shape=(None, 1), dtype=tf.int32)
            t_encoded_gt_boxes = tf.keras.Input(shape=(None, 1, BOX_ENCODING_LEN), dtype=tf.float32)
            t_valid_gt_boxes = tf.keras.Input(shape=(None, 1, 1), dtype=tf.float32)
            t_is_training = tf.keras.Input(shape=(), dtype=tf.bool)
            ...
            t_logits, t_pred_box = model.predict(t_initial_vertex_features, t_vertex_coord_list, t_keypoint_indices_list, t_edges_list, t_is_training)
...
if 'unify_copies' in train_config:
    if train_config['unify_copies']:
        ...
        for ti in range(len(input_tensor_sets)):
            weight = tf.math.divide_no_nan(tf.cast(len(input_tensor_sets) * input_tensor_sets[ti]['t_num_endpoint'], tf.float32), tf.cast(total_num_endpoints, tf.float32))
            weight = tf.cast(weight, tf.float32)
            valid_weight = tf.math.divide_no_nan(tf.cast(len(input_tensor_sets) * input_tensor_sets[ti]['t_num_valid_endpoint'], tf.float32), tf.cast(total_num_valid_endpoints, tf.float32))
            ...
            input_tensor_sets[ti]['t_total_loss'] = input_tensor_sets[ti]['t_cls_loss'] + input_tensor_sets[ti]['t_loc_loss'] + input_tensor_sets[ti]['t_reg_loss']
...
# Using tf.keras.metrics for distributed training
...
for class_idx in range(NUM_CLASSES):
    for bi in range(BOX_ENCODING_LEN):
        ...
        t_mean_loss = tf.keras.metrics.Mean(name=f'loc_loss_cls_{class_idx}_box_{bi}')
        t_mean_loss.update_state(classwise_loc_loss_ind)
        t_classwise_loc_loss_update_ops[f'loc_loss_cls_{class_idx}_box_{bi}'] = t_mean_loss

# metrics
t_recall_update_ops = {}
for class_idx in range(NUM_CLASSES):
    t_recall = tf.keras.metrics.Recall(name=f'recall_{class_idx}')
    t_recall_update_ops[f'recall_{class_idx}'] = t_recall

t_precision_update_ops = {}
for class_idx in range(NUM_CLASSES):
    t_precision = tf.keras.metrics.Precision(name=f'precision_{class_idx}')
    t_precision_update_ops[f'precision_{class_idx}'] = t_precision

t_mAP_update_ops = {}
for class_idx in range(NUM_CLASSES):
    t_mAP = tf.keras.metrics.AUC(curve='PR', name=f'mAP_{class_idx}')
    t_mAP_update_ops[f'mAP_{class_idx}'] = t_mAP

t_mean_cls_loss = tf.keras.metrics.Mean(name='mean_cls_loss')
t_mean_loc_loss = tf.keras.metrics.Mean(name='mean_loc_loss')
t_mean_reg_loss = tf.keras.metrics.Mean(name='mean_reg_loss')
t_mean_total_loss = tf.keras.metrics.Mean(name='mean_total_loss')

metrics_update_ops = {
    'cls_loss': t_mean_cls_loss,
    'loc_loss': t_mean_loc_loss,
    'reg_loss': t_mean_reg_loss,
    'total_loss': t_mean_total_loss
}
...
t_learning_rate = tf.compat.v1.train.exponential_decay(train_config['initial_lr'], global_step, train_config['decay_step'], train_config['decay_factor'], staircase=train_config.get('is_staircase', True))
...
optimizer = optimizer_class(learning_rate=t_learning_rate, **optimizer_kwargs)
grads_cross_gpu = []
with tf.control_dependencies(update_ops):
    for gi in range(NUM_GPU):
        with tf.device('/gpu:%d' % gi):
            grads = optimizer.compute_gradients(input_tensor_sets[gi]['t_total_loss'])
...
if train_config['gpu_memusage'] < 0:
    ...
    if train_config['gpu_memusage'] < -10:
        ...
        gpu_options = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=train_config['gpu_memusage'])
batch_ctr = 0
batch_gradient_list = []
with tf.compat.v1.Session(graph=graph, config=tf.compat.v1.ConfigProto(allow_soft_placement=True, gpu_options=gpu_options)) as sess:
    sess.run(tf.compat.v1.global_variables_initializer())
    states = tf.compat.v1.train.get_checkpoint_state(train_config['train_dir'])
        ...
        for batch_idx in range(0, NUM_TEST_SAMPLE - batch_size + 1, batch_size):
            ...
            if 'max_steps' in train_config and train_config['max_steps'] > 0:
                if results['step'] >= train_config['max_steps']:
                    ...
                    saver.save(sess, checkpoint_path, latest_filename='checkpoint', global_step=results['step'])
                    save_config(config_path, config_complete)
                    save_train_config(train_config_path, train_config)
                    raise SystemExit
        print('STEP: %d, epoch_idx: %d, lr: %f, time cost: %f' % (results['step'], epoch_idx, results['learning_rate'], time.time() - start_time))
        print('cls:%f, loc:%f, reg:%f, loss: %f' % (results['cls_loss'], results['loc_loss'], results['reg_loss'], results['total_loss']))
        for class_idx in range(NUM_CLASSES):
            print('Class_%d: recall=%f, prec=%f, mAP=%f, loc=%f' % (class_idx, results[f'recall_{class_idx}'].result().numpy(), results[f'precision_{class_idx}'].result().numpy(), results[f'mAP_{class_idx}'].result().numpy(), results[f'loc_loss_cls_{class_idx}'].result().numpy()))
            print("         x=%.4f y=%.4f z=%.4f l=%.4f h=%.4f w=%.4f y=%.4f" % (
                results[f'loc_loss_cls_{class_idx}_box_0'].result().numpy(),
                results[f'loc_loss_cls_{class_idx}_box_1'].result().numpy(),
                results[f'loc_loss_cls_{class_idx}_box_2'].result().numpy(),
                results[f'loc_loss_cls_{class_idx}_box_3'].result().numpy(),
                results[f'loc_loss_cls_{class_idx}_box_4'].result().numpy(),
                results[f'loc_loss_cls_{class_idx}_box_5'].result().numpy(),
                results[f'loc_loss_cls_{class_idx}_box_6'].result().numpy()))

        # add summaries ====================================================
        for key, metric in metrics_update_ops.items():
            write_summary_scale(key, metric.result().numpy(), results['step'], train_config['train_dir'])
